...
class LoadImageFromFileAzuriaGeo(BaseTransform):
    """Load an Image with N bands using rasterio package and resizing 
    the image using scikit-learn 

    I added this code in transform method 
    and I had also some attributs (bands_list,number_bands,img_size) in __init__ method
    ...
    def __init__(self,
                 ...
                 imdecode_backend: str = 'rasterio',
                 file_client_args: Optional[dict] = None,
                 ignore_empty: bool = False,
                 bands_list: list =[5,4,3,2,1],
                 number_bands: int = 5,
        ...
        self.img_shape = (number_bands,) + img_size
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if self.imdecode_backend=="rasterio":
            with rasterio.open(filename,mode="r", nodata=0) as src:
                img = src.read(self.bands_list)
        
        img= 255.0*resize(img, self.img_shape,order=1)#np.resize(img,self.img_shape)
